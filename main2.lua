...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        local MaxTitleW = InitialLeftWidth - (WindowInfo.Icon and (WindowInfo.IconSize.X.Offset + 12) or 16)
        local X = math.min(
            Library:GetTextBounds(WindowInfo.Title, Library.Scheme.Font, 17, MaxTitleW),
            MaxTitleW
        )
    ...
    local function ApplyCompact()
        ...
        if not WindowInfo.Icon then
            if WindowInfo.Icon then WindowIcon.Visible = IsCompact end
        end
...
local Window = Library:CreateWindow({
    ...
    Footer = {
        "Nebula Scripts |",
        { Text = "Universal Hub", Copyable = true },
        "|",
        { Text = "v2.5 Ultra", Copyable = false },
        "|",
        { Text = "User: " .. (LocalPlayer and LocalPlayer.DisplayName or "Unknown"), Copyable = true },
    },
    CopyableFooter = true,
...
local Tabs = {
    Player = Window:AddTab({ Name = "Player", Icon = "user", Description = "Local player movement, physics, speed and survival" }),
    Combat = Window:AddTab({ Name = "Combat", Icon = "crosshair", Description = "Aimbot, silent aim, triggerbot, kill aura and hitboxes" }),
    Visuals = Window:AddTab({ Name = "Visuals", Icon = "eye", Description = "ESP players, crosshair, self chams, x-ray and lighting" }),
    World = Window:AddTab({ Name = "World", Icon = "globe", Description = "Teleports, player flings, black hole and waypoints" }),
    Utility = Window:AddTab({ Name = "Utility", Icon = "wrench", Description = "Anti-kick, dex, simplespy, rejoin and performance boost" }),
    Settings = Window:AddTab({ Name = "UI Settings", Icon = "settings", Description = "Configure menu, themes, keybinds and scale" }),
}
...
--// ==============================================================================
do
    Tabs.Player:AddPlayerInfo("PlayerBanner", {
        Title = "Welcome to <b>Nebula</b> — Universal Hub",
        Description = {
            '<font color="#8f6bff">Movement</font> — WalkSpeed, CFrame Bypass, Jump, Noclip, Fly, Bhop',
            '<font color="#8f6bff">Combat</font> — Camera Aimbot, Silent Aim, Fast Clicker, Kill Aura',
            '<font color="#8f6bff">Visuals</font> — 3D Highlights, Tracers, Crosshair, Self Chams, X-Ray',
            '<font color="#8f6bff">World & Tools</font> — Fling Player, Black Hole, Anti-Kick, Dex, SimpleSpy',
        },
    ...
    Movement:AddToggle("CFrameSpeed", {
        Text = "CFrame Speed (Anti-Cheat Bypass)",
        Default = false,
    })

    Movement:AddSlider("CFrameSpeedValue", {
    ...
    Movement:AddToggle("JumpPowerEnabled", {
    ...
    Movement:AddToggle("ClickTP", {
        Text = "Click Teleport (Ctrl + Click)",
        Default = false,
    })

    local Survival = Tabs.Player:AddRightGroupbox("Survival & Anti-Aim", "shield")

    Survival:AddToggle("AntiAfk", {
        Text = "Anti-AFK (Idle Bypass)",
        Default = true,
    })

    Survival:AddToggle("AntiVoid", {
        Text = "Anti-Void Fallback",
        Default = true,
    })

    Survival:AddSlider("AntiVoidY", {
        Text = "Fall Threshold",
        Default = -100,
        Min = -500,
        Max = -20,
    ...
    Survival:AddToggle("NoFallDamage", {
        Text = "Anti-Fall Velocity Damper",
        Default = false,
    })

    Survival:AddDivider()

    Survival:AddToggle("Spinbot", {
        Text = "Spinbot (Yaw Anti-Aim)",
        Default = false,
    })

    Survival:AddSlider("SpinSpeed", {
        Text = "Spinbot Speed",
        Default = 25,
        Min = 5,
        Max = 100,
    ...
    Survival:AddButton({
        Text = "Teleport to Spawn (0, 50, 0)",
        Tooltip = "Emergency reset position",
        Func = function()
            local char = LocalPlayer.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if hrp then
                hrp.CFrame = CFrame.new(0, 50, 0)
                Library:Notify({ Title = "Spawn", Description = "Teleported to (0, 50, 0)", Time = 3 })
            end
        end,
    })
end
...
--// ==============================================================================
do
    local AimbotBox = Tabs.Combat:AddLeftGroupbox("Aimbot & Targeting", "crosshair")
    ...
    AimbotBox:AddToggle("SilentAim", {
        Text = "Silent Aim (Bullet Redirection)",
        Default = false,
    })

    AimbotBox:AddSlider("SilentHitChance", {
    ...
    AimbotBox:AddDropdown("AimbotBone", {
        Text = "Target Bone",
    ...
    AimbotBox:AddSlider("AimbotDist", {
        Text = "Max Distance",
        Default = 1000,
        Min = 50,
        Max = 4000,
    ...
    AimbotBox:AddToggle("AimbotFOVCheck", {
        Text = "Check FOV Radius",
        Default = true,
    })

    local CombatModBox = Tabs.Combat:AddRightGroupbox("Combat Tools & FOV", "target")

    CombatModBox:AddToggle("ShowFOV", {
        Text = "Draw FOV Circle",
        Default = false,
    }):AddColorPicker("FOVColor", {
        Default = Color3.fromRGB(143, 107, 255),
        Title = "FOV Circle Color",
    })

    CombatModBox:AddSlider("FOVRadius", {
        Text = "FOV Radius",
        Default = 140,
        Min = 20,
        Max = 600,
    ...
    CombatModBox:AddDivider()

    CombatModBox:AddToggle("AutoClicker", {
        Text = "Fast Auto Clicker (M1 / Tool)",
        Default = false,
    }):AddKeyPicker("AutoClickerKey", {
        Default = "V",
        Mode = "Toggle",
        Text = "Clicker Key",
    })

    CombatModBox:AddSlider("ClickerCPS", {
    ...
    CombatModBox:AddToggle("Triggerbot", {
        Text = "Triggerbot (Auto Click on Target)",
        Default = false,
    })

    CombatModBox:AddSlider("TriggerbotDelay", {
    ...
    CombatModBox:AddToggle("KillAura", {
        Text = "Melee Kill Aura",
        Default = false,
    })

    CombatModBox:AddSlider("AuraRange", {
        Text = "Aura Reach",
        Default = 15,
        Min = 5,
        Max = 35,
    ...
    CombatModBox:AddToggle("AuraTeamCheck", {
        Text = "Aura Team Check",
        Default = true,
    })

    CombatModBox:AddDivider()

    CombatModBox:AddToggle("HitboxExpander", {
        Text = "Hitbox Expander",
        Default = false,
    })

    CombatModBox:AddSlider("HitboxSize", {
        Text = "Hitbox Size",
        Default = 10,
        Min = 2,
        Max = 35,
    ...
    CombatModBox:AddToggle("HitboxTeamCheck", {
...
--// ==============================================================================
do
    local EspBox = Tabs.Visuals:AddLeftGroupbox("Player ESP", "eye")

    EspBox:AddToggle("EspBox", {
        Text = "Player Highlights / Box",
        Default = false,
    ...
    EspBox:AddToggle("EspTracers", {
        ...
        Default = Color3.fromRGB(255, 210, 60),
        Title = "Tracer Color",
    })
    ...
    local WorldBox = Tabs.Visuals:AddRightGroupbox("World, Crosshair & Self", "sun")

    WorldBox:AddToggle("SelfChams", {
        Text = "Self Chams / Glow",
        Default = false,
    }):AddColorPicker("SelfChamsColor", {
        Default = Color3.fromRGB(143, 107, 255),
        Title = "Self Glow Color",
    })

    WorldBox:AddToggle("SelfRainbow", {
        Text = "Self Rainbow Mode",
        Default = false,
    })

    WorldBox:AddDivider()

    WorldBox:AddToggle("CustomCrosshair", {
        Text = "Custom Screen Crosshair",
        Default = false,
    ...
    WorldBox:AddToggle("XRay", {
        Text = "X-Ray Vision (Transparent Walls)",
        Default = false,
    })
    ...
    WorldBox:AddSlider("FieldOfView", {
        Text = "Camera FOV",
        Default = 70,
        Min = 30,
        Max = 120,
    ...
    WorldBox:AddSlider("MaxZoom", {
        Text = "Max Camera Zoom",
        Default = 128,
        Min = 50,
        Max = 10000,
        Rounding = 0,
...
local function flingTargetPlayer(targetPlayer)
    ...
    Library:Notify({ Title = "Fling", Description = "Flinging " .. targetPlayer.Name .. "...", Time = 2 })

    task.spawn(function()
        local startTime = tick()
        local bav = Instance.new("BodyAngularVelocity")
        bav.Name = "NebulaFlingBAV"
        ...
        while tick() - startTime < 1.5 do
            if not targetPlayer.Character or not tHrp.Parent or not myHrp.Parent then break end
        ...
        myHrp.CFrame = origCF
        Library:Notify({ Title = "Fling Complete", Description = "Restored original position!", Time = 2 })
    end)
end

do
    local TeleportBox = Tabs.World:AddLeftGroupbox("Player Teleport & Fling", "navigation")

    PlayerDropdown = TeleportBox:AddDropdown("SelectedPlayer", {
        Text = "Target Player",
    ...
    TeleportBox:AddButton({
        ...
        Tooltip = "Refresh names of online players",
        Func = function()
            PlayerDropdown:SetValues(getPlayerList())
            Library:Notify({ Title = "Players", Description = "Player list refreshed!", Time = 2 })
        end,
    })

    TeleportBox:AddButton({
        Text = "Teleport to Player",
        Tooltip = "Teleports directly behind selected player",
        Func = function()
            local targetName = Options.SelectedPlayer.Value
            local targetPlayer = Players:FindFirstChild(targetName)
            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local myChar = LocalPlayer.Character
                local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart")
                if myHrp then
                    myHrp.CFrame = targetPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 3)
                    Library:Notify({ Title = "Teleport", Description = "Teleported to " .. targetName, Time = 3 })
                end
            else
                Library:Notify({ Title = "Teleport Error", Description = "Player not found or character missing", Time = 3 })
            end
        end,
    })
    ...
    TeleportBox:AddButton({
        ...
        Tooltip = "Launches target player with high velocity angular spin",
        Func = function()
            local targetName = Options.SelectedPlayer.Value
            local targetPlayer = Players:FindFirstChild(targetName)
            if targetPlayer then
                ...
                Library:Notify({ Title = "Fling Error", Description = "No player selected", Time = 3 })
            end
        end,
    })

    TeleportBox:AddToggle("TouchFling", {
        Text = "Touch Fling (Launch On Contact)",
        Default = false,
    })

    TeleportBox:AddToggle("SpectatePlayer", {
        Text = "Spectate Player",
        Default = false,
    })

    local WaypointsBox = Tabs.World:AddRightGroupbox("Waypoints, Physics & Orbit", "map-pin")

    SavedPosLabel = WaypointsBox:AddLabel({ Text = "Saved Pos: None", DoesWrap = true })

    WaypointsBox:AddButton({
        Text = "Save Current Position",
        Tooltip = "Records your current coordinates",
        Func = function()
            local char = LocalPlayer.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if hrp then
                ...
                SavedPosLabel:SetText(string.format("Saved Pos: %.0f, %.0f, %.0f", pos.X, pos.Y, pos.Z))
                Library:Notify({ Title = "Waypoint Saved", Description = "Current position recorded!", Time = 3 })
            end
        end,
    })
    ...
    WaypointsBox:AddButton({
        ...
        Tooltip = "Warp to your saved waypoint",
        Func = function()
            if savedPosition then
                ...
                if hrp then
                    ...
                    Library:Notify({ Title = "Waypoint", Description = "Warped to saved waypoint!", Time = 3 })
                end
            else
                Library:Notify({ Title = "Waypoint Error", Description = "No position has been saved yet!", Time = 3 })
            end
        end,
    })

    WaypointsBox:AddDivider()

    WaypointsBox:AddToggle("BlackHole", {
        Text = "Black Hole (Pull Unanchored Parts)",
        Default = false,
    })

    WaypointsBox:AddSlider("BlackHoleRadius", {
        Text = "Orbit Radius",
        Default = 10,
        Min = 3,
        Max = 40,
    ...
    WaypointsBox:AddSlider("WorldGravity", {
    ...
    WaypointsBox:AddButton({
        Text = "Reset Gravity to Default (196.2)",
        Func = function()
            Options.WorldGravity:SetValue(196.2)
        end,
    })

    WaypointsBox:AddButton({
        Text = "Give Client BTools (HopperBins)",
        Tooltip = "Adds Clone, Hammer, and Grab hopper tools to your backpack",
        Func = function()
            ...
            if backpack then
                ...
                Library:Notify({ Title = "BTools", Description = "Universal Hopper BTools added!", Time = 3 })
            end
        end,
    })
end
...
--// ==============================================================================
do
    local ServerBox = Tabs.Utility:AddLeftGroupbox("Server & Exploitation", "server")

    ServerBox:AddToggle("AntiKick", {
        Text = "Anti-Kick Protection",
        Default = true,
    })

    ServerBox:AddButton({
        Text = "Rejoin Current Server",
        Tooltip = "Reconnect to the same server instance",
        Func = function()
    ...
    ServerBox:AddButton({
        Text = "Server Hop (Next Server)",
        Tooltip = "Find and join a different active server",
        Func = function()
            Library:Notify({ Title = "Server Hop", Description = "Scanning public servers...", Time = 4 })
            task.spawn(function()
    ...
    ServerBox:AddButton({
        ...
        Tooltip = "Copies current server JobId to clipboard",
        Func = function()
            local clip = setclipboard or toclipboard or (syn and syn.write_clipboard)
            if clip then
                clip(tostring(game.JobId))
                Library:Notify({ Title = "Clipboard", Description = "JobId copied to clipboard!", Time = 3 })
            end
        end,
    })

    ServerBox:AddButton({
        Text = "Copy Game PlaceId",
        Tooltip = "Copies current PlaceId to clipboard",
        Func = function()
            local clip = setclipboard or toclipboard or (syn and syn.write_clipboard)
            if clip then
                clip(tostring(game.PlaceId))
                Library:Notify({ Title = "Clipboard", Description = "PlaceId copied to clipboard!", Time = 3 })
            end
        end,
    })
    ...
    ServerBox:AddButton({
        Text = "Load Dark Dex V3",
        Tooltip = "Opens Dex Explorer interface",
        Func = function()
            Library:Notify({ Title = "Dex", Description = "Loading Dark Dex V3...", Time = 3 })
            task.spawn(function()
                pcall(function()
    ...
    ServerBox:AddButton({
        Text = "Load SimpleSpy V3",
        Tooltip = "Opens Remote Spy logger",
        Func = function()
            Library:Notify({ Title = "SimpleSpy", Description = "Loading SimpleSpy...", Time = 3 })
            task.spawn(function()
                pcall(function()
                    loadstring(game:HttpGet("https://raw.githubusercontent.com/exxtremestuffs/SimpleSpy/master/source.lua"))()
                end)
            end)
        end,
    })
    ...
    ServerBox:AddButton({
        Text = "Load Infinite Yield FE",
        Tooltip = "Opens Infinite Yield command suite",
        Func = function()
            Library:Notify({ Title = "Infinite Yield", Description = "Loading Infinite Yield...", Time = 3 })
            task.spawn(function()
                pcall(function()
    ...
    local PerfBox = Tabs.Utility:AddRightGroupbox("Performance & Boost", "zap")
    ...
    PerfBox:AddButton({
        Text = "Unlock FPS (setfpscap 999)",
        Tooltip = "Uncaps frame rate if executor supports setfpscap",
        Func = function()
            if setfpscap then
                setfpscap(999)
                Library:Notify({ Title = "FPS Boost", Description = "FPS cap set to 999!", Time = 3 })
            else
                Library:Notify({ Title = "Not Supported", Description = "Executor does not support setfpscap", Time = 3 })
            end
        end,
    })

    PerfBox:AddButton({
        Text = "Boost FPS / Anti-Lag Mode",
        Tooltip = "Clears textures, decals, and sets smooth plastic",
        Func = function()
            local count = 0
            ...
            Library:Notify({ Title = "Boost Applied", Description = "Optimized " .. count .. " world elements!", Time = 4 })
        end,
    })

    PerfBox:AddButton({
        Text = "Clear Map Debris & Sounds",
        Tooltip = "Cleans up client debris and stops sound lag",
        Func = function()
            for _, s in ipairs(workspace:GetDescendants()) do
                if s:IsA("Sound") and not s.Playing then
                    s:Destroy()
                end
            end
            Library:Notify({ Title = "Cleaned", Description = "Client sound cache cleaned!", Time = 3 })
        end,
    })
end
...
--// ==============================================================================
do
    local MenuBox = Tabs.Settings:AddLeftGroupbox("Menu Settings", "wrench")
    ...
    MenuBox:AddSlider("CornerRadius", {
        Text = "Corner Radius",
    ...
    local InfoBox = Tabs.Settings:AddRightGroupbox("Information", "info")
    InfoBox:AddLabel("Nebula Free Script Hub")
    InfoBox:AddLabel("General Roblox Edition v2.5", true)
    InfoBox:AddDivider()
    InfoBox:AddLabel("Built with ObsidianUltra UI")
    InfoBox:AddLabel("Press [RightShift] to hide/show menu")
    InfoBox:AddLabel("All features active & operational")
end

--// ==============================================================================
--// 7. REAL EXECUTION BACKEND ENGINE
--// ==============================================================================

--// A. Player Connections
...
local function removeEsp(player)
    if espTable[player] then
        if espTable[player].Highlight then pcall(function() espTable[player].Highlight:Destroy() end) end
        if espTable[player].Billboard then pcall(function() espTable[player].Billboard:Destroy() end) end
        if espTable[player].Tracer then pcall(function() espTable[player].Tracer:Remove() end) end
        espTable[player] = nil
    end
end
...
--// B. Movement, Speed, Jump, Noclip & Anti-Fall
RunService.Stepped:Connect(function()
    ...
    -- WalkSpeed
    if hum and Toggles.WalkSpeedEnabled and Toggles.WalkSpeedEnabled.Value then
        hum.WalkSpeed = Options.WalkSpeed.Value
    end

    -- JumpPower
    if hum and Toggles.JumpPowerEnabled and Toggles.JumpPowerEnabled.Value then
    ...
    -- Anti-Fall Velocity Damper
    if Toggles.NoFallDamage and Toggles.NoFallDamage.Value and hrp then
...
--// C. CFrame Speed, BunnyHop & Spinbot
RunService.Heartbeat:Connect(function()
    ...
    -- CFrame Speed
    if Toggles.CFrameSpeed and Toggles.CFrameSpeed.Value and hrp and hum then
        local moveDir = hum.MoveDirection
        if moveDir.Magnitude > 0 then
            local factor = (Options.CFrameSpeedValue and Options.CFrameSpeedValue.Value or 2) * 0.15
            hrp.CFrame = hrp.CFrame + (moveDir * factor)
        end
    end

    -- BunnyHop
    if Toggles.BunnyHop and Toggles.BunnyHop.Value and hum and hum.MoveDirection.Magnitude > 0 then
    ...
    -- Spinbot
    if Toggles.Spinbot and Toggles.Spinbot.Value and hrp then
        local speed = Options.SpinSpeed and Options.SpinSpeed.Value or 25
...
RunService.RenderStepped:Connect(function()
    ...
    if Toggles.FlyEnabled and Toggles.FlyEnabled.Value and hrp and hum and hum.Health > 0 then
        if not flyBV or flyBV.Parent ~= hrp then
            cleanupFly()

            flyBV = Instance.new("BodyVelocity")
            flyBV.Name = "NebulaFlyBV"
            flyBV.MaxForce = Vector3.new(9e9, 9e9, 9e9)
            flyBV.Velocity = Vector3.zero
            flyBV.Parent = hrp

            flyBG = Instance.new("BodyGyro")
            flyBG.Name = "NebulaFlyBG"
...
--// F. Click Teleport (Ctrl + Click)
UserInputService.InputBegan:Connect(function(input, processed)
    if processed then return end
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        if Toggles.ClickTP and Toggles.ClickTP.Value and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
            local char = LocalPlayer.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
...
local safeVoidPosition = Vector3.new(0, 50, 0)
RunService.Heartbeat:Connect(function()
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    if hrp then
        if hrp.Position.Y > 0 then
            safeVoidPosition = hrp.Position + Vector3.new(0, 2, 0)
        end
        if Toggles.AntiVoid and Toggles.AntiVoid.Value then
            local threshold = Options.AntiVoidY and Options.AntiVoidY.Value or -100
            if hrp.Position.Y < threshold then
                hrp.Velocity = Vector3.zero
                hrp.CFrame = CFrame.new(safeVoidPosition)
            end
        end
    end
end)

--// H. Combat & Aimbot Engine
local fovCircle = nil
pcall(function()
...
local function getClosestPlayer()
    ...
    local maxDist = Options.AimbotDist and Options.AimbotDist.Value or 1000
    local fovRadius = Options.FOVRadius and Options.FOVRadius.Value or 140
    local checkFOV = Toggles.AimbotFOVCheck and Toggles.AimbotFOVCheck.Value
    local checkTeam = Toggles.AimbotTeamCheck and Toggles.AimbotTeamCheck.Value
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            if not checkTeam or (player.Team == nil or player.Team ~= LocalPlayer.Team) then
                local char = player.Character
                local hum = char and char:FindFirstChildOfClass("Humanoid")
                local part = char and (char:FindFirstChild(boneName) or char:FindFirstChild("Head") or char:FindFirstChild("HumanoidRootPart"))
                ...
                if hum and hum.Health > 0 and part then
                    ...
                    if worldDist <= maxDist then
                        ...
                        if onScreen then
                            ...
                            if not checkFOV or screenDist <= fovRadius then
                                local isVisible = true
                                if checkWall then
                                    local rayParams = RaycastParams.new()
                                    rayParams.FilterDescendantsInstances = { myChar, workspace.CurrentCamera }
                                    rayParams.FilterType = Enum.RaycastFilterType.Exclude
                                    local result = workspace:Raycast(workspace.CurrentCamera.CFrame.Position, (part.Position - workspace.CurrentCamera.CFrame.Position), rayParams)
                                    if result and not result.Instance:IsDescendantOf(char) then
                                        isVisible = false
                                    end
                                end

                                if isVisible and screenDist < shortestDist then
...
pcall(function()
    if hookmetamethod and getrawmetatable then
        ...
        rawMeta.__index = newcclosure(function(self, key)
            if not checkcaller() and Toggles.SilentAim and Toggles.SilentAim.Value then
                local hitChance = Options.SilentHitChance and Options.SilentHitChance.Value or 100
                if math.random(1, 100) <= hitChance then
                    if self == Mouse and (key == "Hit" or key == "hit") then
                        local target = getClosestPlayer()
                        if target then return target.CFrame end
                    elseif self == Mouse and (key == "Target" or key == "target") then
                        local target = getClosestPlayer()
...
RunService.RenderStepped:Connect(function()
    ...
    if fovCircle then
        if Toggles.ShowFOV and Toggles.ShowFOV.Value then
            ...
            fovCircle.Radius = Options.FOVRadius and Options.FOVRadius.Value or 140
            fovCircle.Color = Options.FOVColor and Options.FOVColor.Value or Color3.fromRGB(143, 107, 255)
    ...
    if Toggles.AimbotEnabled and Toggles.AimbotEnabled.Value and Options.AimbotKey:GetState() then
        ...
        if targetPart then
            ...
            local smooth = Options.AimbotSmooth and Options.AimbotSmooth.Value or 1
            if smooth <= 1 then
...
--// I. Auto Clicker, Kill Aura, Triggerbot & Hitbox Expander
local lastTriggerTick = 0
local lastClickerTick = 0
...
RunService.Heartbeat:Connect(function()
    ...
    -- Fast Auto Clicker
    if Toggles.AutoClicker and Toggles.AutoClicker.Value then
        local now = tick()
        local interval = 1 / (Options.ClickerCPS and Options.ClickerCPS.Value or 15)
        if now - lastClickerTick >= interval then
            lastClickerTick = now
            pcall(function()
    ...
    -- Melee Kill Aura
    if Toggles.KillAura and Toggles.KillAura.Value and myHrp then
        local now = tick()
        if now - lastAuraTick >= 0.1 then
            local range = Options.AuraRange and Options.AuraRange.Value or 15
            local teamCheck = Toggles.AuraTeamCheck and Toggles.AuraTeamCheck.Value
            local tool = myChar and myChar:FindFirstChildOfClass("Tool")
            for _, player in ipairs(Players:GetPlayers()) do
                if player ~= LocalPlayer and player.Character then
                    ...
                    if pHrp and pHum and pHum.Health > 0 then
                        local isEnemy = not teamCheck or (player.Team == nil or player.Team ~= LocalPlayer.Team)
                        if isEnemy and (pHrp.Position - myHrp.Position).Magnitude <= range then
                            lastAuraTick = now
                            if tool then
                                tool:Activate()
                                local handle = tool:FindFirstChild("Handle")
                                if handle and firetouchinterest then
                                    firetouchinterest(handle, pHrp, 0)
                                    firetouchinterest(handle, pHrp, 1)
                                end
                            else
    ...
    if Toggles.Triggerbot and Toggles.Triggerbot.Value then
        local now = tick()
        local delay = ((Options.TriggerbotDelay and Options.TriggerbotDelay.Value) or 80) / 1000
        if now - lastTriggerTick >= delay then
    ...
    -- Hitbox Expander
    local hbEnabled = Toggles.HitboxExpander and Toggles.HitboxExpander.Value
    local hbSize = Options.HitboxSize and Options.HitboxSize.Value or 10
    local hbTeam = Toggles.HitboxTeamCheck and Toggles.HitboxTeamCheck.Value
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            if hrp then
                local isEnemy = not hbTeam or (player.Team == nil or player.Team ~= LocalPlayer.Team)
                if hbEnabled and isEnemy then
    ...
    -- Touch Fling
    if Toggles.TouchFling and Toggles.TouchFling.Value and myHrp then
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character then
                local pHrp = p.Character:FindFirstChild("HumanoidRootPart")
                if pHrp and (pHrp.Position - myHrp.Position).Magnitude <= 4 then
                    myHrp.Velocity = Vector3.new(0, 50000, 0)
                    myHrp.RotVelocity = Vector3.new(50000, 50000, 50000)
                    break
                end
            end
        end
    ...
    -- Black Hole (Pull Unanchored Parts)
    if Toggles.BlackHole and Toggles.BlackHole.Value and myHrp then
        local radius = Options.BlackHoleRadius and Options.BlackHoleRadius.Value or 10
        for _, part in ipairs(workspace:GetDescendants()) do
            if part:IsA("BasePart") and not part.Anchored and not (myChar and part:IsDescendantOf(myChar)) then
                local offset = Vector3.new(
                    math.sin(tick() * 4 + part:GetHashCode() % 10) * radius,
                    math.cos(tick() * 4) * 2,
                    math.cos(tick() * 4 + part:GetHashCode() % 10) * radius
                )
                part.CFrame = myHrp.CFrame * CFrame.new(offset)
...
--// J. Visuals: ESP System, Crosshair & Self Chams
...
RunService.RenderStepped:Connect(function()
    ...
    -- Self Chams
    if Toggles.SelfChams and Toggles.SelfChams.Value and myChar then
        if not selfHighlight or selfHighlight.Parent ~= myChar then
            if selfHighlight then selfHighlight:Destroy() end
            selfHighlight = Instance.new("Highlight")
            selfHighlight.Name = "NebulaSelfHighlight"
            selfHighlight.FillTransparency = 0.4
            selfHighlight.OutlineTransparency = 0
            selfHighlight.Adornee = myChar
            selfHighlight.Parent = myChar
        end
        local col = (Toggles.SelfRainbow and Toggles.SelfRainbow.Value) and rainbowColor or (Options.SelfChamsColor and Options.SelfChamsColor.Value or Color3.fromRGB(143, 107, 255))
        selfHighlight.FillColor = col
    ...
    if #crosshairLines == 4 then
        if Toggles.CustomCrosshair and Toggles.CustomCrosshair.Value then
            local size = Options.CrosshairSize and Options.CrosshairSize.Value or 12
            local gap = Options.CrosshairGap and Options.CrosshairGap.Value or 4
            local col = Options.CrosshairColor and Options.CrosshairColor.Value or Color3.fromRGB(143, 107, 255)

            -- Top line
            crosshairLines[1].From = Vector2.new(center.X, center.Y - gap)
            crosshairLines[1].To = Vector2.new(center.X, center.Y - gap - size)
            crosshairLines[1].Color = col
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            local isAlive = hum and hum.Health > 0 and hrp and head and myHrp
            local dist = isAlive and (hrp.Position - myHrp.Position).Magnitude or 999999
            local maxDist = Options.EspMaxDist and Options.EspMaxDist.Value or 2000
            local teamCheck = Toggles.EspTeamCheck and Toggles.EspTeamCheck.Value
            local isEnemy = not teamCheck or (player.Team == nil or player.Team ~= LocalPlayer.Team)

            if isAlive and dist <= maxDist and isEnemy then
                if not espTable[player] then
                    espTable[player] = {}
                end
                local entry = espTable[player]
                ...
                if Toggles.EspBox and Toggles.EspBox.Value then
                    if not entry.Highlight or entry.Highlight.Parent ~= char then
                        ...
                        entry.Highlight.Name = "NebulaHighlight"
                    ...
                    local col = (Toggles.EspRainbow and Toggles.EspRainbow.Value) and rainbowColor or (Options.EspBoxColor and Options.EspBoxColor.Value or Color3.fromRGB(255, 65, 65))
                ...
                -- 2. BillboardGui (Names & Distance & Health)
                ...
                if showNames or showHealth then
                    if not entry.Billboard or entry.Billboard.Parent ~= head then
                        if entry.Billboard then entry.Billboard:Destroy() end
                        entry.Billboard = Instance.new("BillboardGui")
                        entry.Billboard.Name = "NebulaEspGui"
                        entry.Billboard.Size = UDim2.new(0, 200, 0, 50)
                        entry.Billboard.StudsOffset = Vector3.new(0, 2.5, 0)
                        entry.Billboard.AlwaysOnTop = true
                        entry.Billboard.Adornee = head
                        entry.Billboard.Parent = head
                        ...
                        text.Parent = entry.Billboard
                    end

                    local textLabel = entry.Billboard:FindFirstChild("EspText")
                    if textLabel then
                        local displayText = ""
                        if showNames then
                            displayText = string.format("%s [%.0fm]", player.DisplayName, dist)
                        end
                        if showHealth then
                            local hpPct = math.clamp(hum.Health / (hum.MaxHealth > 0 and hum.MaxHealth or 100), 0, 1)
                            local hpText = string.format(" [%.0f%%]", hpPct * 100)
                            displayText = displayText .. hpText
                        end
                        local nameCol = (Toggles.EspRainbow and Toggles.EspRainbow.Value) and rainbowColor or (Options.EspNameColor and Options.EspNameColor.Value or Color3.fromRGB(255, 255, 255))
                        textLabel.TextColor3 = nameCol
                ...
                -- 3. Tracers
                if Toggles.EspTracers and Toggles.EspTracers.Value and Drawing and Drawing.new then
                    ...
                    if entry.Tracer then
                        ...
                        if onScreen then
                            local tracerCol = (Toggles.EspRainbow and Toggles.EspRainbow.Value) and rainbowColor or (Options.EspTracerColor and Options.EspTracerColor.Value or Color3.fromRGB(255, 210, 60))
                            entry.Tracer.Color = tracerCol
                            entry.Tracer.From = Vector2.new(viewportSize.X / 2, viewportSize.Y)
                ...
                if espTable[player] then
                    if espTable[player].Highlight then espTable[player].Highlight.Enabled = false end
                    if espTable[player].Billboard then espTable[player].Billboard.Enabled = false end
                    if espTable[player].Tracer then espTable[player].Tracer.Visible = false end
                end
            end
        end
    end
...
--// K. Environment, X-Ray & World Listeners
...
local xrayOriginals = {}
Toggles.XRay:OnChanged(function(enabled)
    if enabled then
        for _, part in ipairs(workspace:GetDescendants()) do
...
Toggles.Fullbright:OnChanged(function(enabled)
    if enabled then
...
Toggles.RemoveFog:OnChanged(function(enabled)
    if enabled then
        Lighting.FogEnd = 9e9
...
Options.TimeOfDay:OnChanged(function(val)
    Lighting.ClockTime = val
end)

Options.FieldOfView:OnChanged(function(val)
    workspace.CurrentCamera.FieldOfView = val
end)

Options.MaxZoom:OnChanged(function(val)
    LocalPlayer.CameraMaxZoomDistance = val
end)

Options.WorldGravity:OnChanged(function(val)
    workspace.Gravity = val
end)

Options.FPSCap:OnChanged(function(val)
    if setfpscap then
        pcall(function() setfpscap(val) end)
    end
end)

Toggles.SpectatePlayer:OnChanged(function(enabled)
    local cam = workspace.CurrentCamera
    if enabled then
        local targetName = Options.SelectedPlayer.Value
        local targetPlayer = Players:FindFirstChild(targetName)
        ...
        if hum then
            ...
            Library:Notify({ Title = "Spectate", Description = "Now spectating " .. targetName, Time = 3 })
        else
            Library:Notify({ Title = "Spectate Error", Description = "Target player or character not found", Time = 3 })
    ...
    if hookmetamethod and getnamecallmethod then
        ...
        oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
            ...
            if not checkcaller() and (method == "Kick" or method == "kick") and self == LocalPlayer then
                if Toggles.AntiKick and Toggles.AntiKick.Value then
                    Library:Notify({ Title = "Anti-Kick", Description = "Blocked a server kick attempt!", Time = 4 })
                    return nil
                end
            end
...
--// N. Watermark & Startup Notification
pcall(function()
    local Watermark = Library:AddWatermark({
        { Text = "Nebula", Accent = true },
        { Text = "Universal Hub" },
        ...
        { Icon = "clock", Text = function() return os.date("%H:%M:%S") end },
    })
    Watermark.RefreshRate = 1
end)

Library:Notify({
    Title = "Nebula | Universal Hub v2.5",
    Description = "Menu charge avec succes ! Appuie sur [RightShift] pour masquer.",
    Time = 5,
})
